...
from brus_backend_common.models import DEFCDeltaInt
from brus_backend_common.helpers.spark_helper import SparkScriptSession
from brus_backend_common.helpers.pandas_helper import clean_data, check_dataframe_diff
from brus_backend_common.helpers.script_helper import (
    exit_if_nonlocal,
...
DEFC_BUCKET = CONFIG.DATA_SOURCES_BUCKET
...
def get_defc_file():
    """Retrieves the defc file to load
    ...
    if CONFIG.IS_LOCAL:
        raise RuntimeError("Unable to retrieve raw DEFC file. Must set IS_LOCAL and AWS config.")
...
def main(local_file=None, force_reload=False):
    s3 = boto3.client("s3", region_name=CONFIG.AWS_REGION)

    with SparkScriptSession() as spark:
        model = DEFCDeltaInt(spark=spark)
        ...
        defc_file = get_defc_file() if not local_file else local_file
        ...
        diff_found = check_dataframe_diff(data, model, ["defc_id"], ["code"], date_format="%Y-%m-%d")
        if force_reload or diff_found:

            # The only diff should be whenever a new code is added. Noting it here
            if diff_found:
                incoming_defcs = list(data["code"])
                curr_defcs = list(model.to_pandas_df()["code"])
    ...
    total_defc_count = DEFCDeltaInt.count()
...
if __name__ == "__main__":
    ...
    main(args.local_file, args.force_reload)
